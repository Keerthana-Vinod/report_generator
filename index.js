...
const generateDailyReport = async (chatId, dateStr) => {
  ...
  for (const poll of groupPolls) {
    if (!authorPolls[poll.authorName]) {
      authorPolls[poll.authorName] = [];
    }
    authorPolls[poll.authorName].push(poll);
  }

  let report = '';
  const authorEntries = Object.entries(authorPolls);
  
  for (let idx = 0; idx < authorEntries.length; idx++) {
    const [authorName, polls] = authorEntries[idx];
    report += `📋 *${authorName} Today's Work*\n\n`;

    const allTasks = [];
    for (const poll of polls) {
      allTasks.push(...poll.tasks);
    }
    ...
    for (const task of allTasks) {
      let statusEmoji = '❌';
      const cleanName = task.optionName.replace(/^\d+[\.\)]\s*/, '');
      ...
      if (task.completed || task.status === 'review') {
        ...
        statusEmoji = '🟡';
        progressVal = task.percent || 0;
        displayName = `${cleanName} (${progressVal}%)`;
      } else if (task.status === 'blocked') {
        statusEmoji = '🟠';
        progressVal = task.percent || 0;
        const reasonStr = task.reason ? ` (${task.reason})` : '';
        displayName = `${cleanName}${reasonStr}`;
      } else {
        statusEmoji = '❌';
        progressVal = 0;
      }
      ...
      report += `${statusEmoji} ${displayName}\n\n`;
    }

    const overallProgress = allTasks.length > 0 ? Math.round(totalProgress / allTasks.length) : 0;

    report += `━━━━━━━━━━━━━━━\n\n`;
    report += `*Completion*\n${completedCount}/${allTasks.length} Completed\n\n`;
    report += `*Overall Progress*\n${overallProgress}%\n\n`;
    report += `_Generated by WhatsApp Task Automation Bot_`;
    
    if (idx < authorEntries.length - 1) {
      report += `\n\n===============================\n\n`;
    }
  }
